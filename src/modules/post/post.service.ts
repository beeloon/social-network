...
export class PostService {
 //  private posts: IPost[] = [];
  ...
  async insertPost(createPostData: CreatePostDto): Promise<Post> {
    try {
      const id = uuidv4();
      const post = this.postRepository.save(createPostData, id )
     // const post1 = this.postRepository.create(id, createPost);
